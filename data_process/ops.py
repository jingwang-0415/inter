    ...
    def gen_components(self):
        ...
        for g in tqdm(range(len(g_list)), desc='Gen Components', unit='graph'):
            ...
            for e in edges:
                ...
                edge = ((node_labels[g][e[0]],node_labels[g][e[1]]), weight)
                ...
                for i in range(e+1, len(edges)):
                    for j in edges[e]:
                        if j in edges[i]:
                            ...
                            weight = g_list[g].get_edge_data(a,b)['weight']
                            edge = ((self.nodes_labels[g][edges[e][0]], self.nodes_labels[g][edges[e][1]]), weight)
                            weight_i = g_list[g].get_edge_data(edges[i][0], edges[i][1])['weight']
                            edge_i = ((self.nodes_labels[g][edges[i][0]], self.nodes_labels[g][edges[i][1]]), weight_i)
                    ...
                    for j in mcb_tuple[m]:
                        if j in mcb_tuple[i]:
                            ...
                            for t in range(len(mcb_tuple[m])):
                                ring.append(self.nodes_labels[g][mcb_tuple[m][t]])
                            cycle = (tuple(ring), weight)

                            weight_i = tuple(self.find_ring_weights(mcb_tuple[i], g_list[g]))
                            ring_i = []
                            for t in range(len(mcb_tuple[i])):
                                ring_i.append(self.nodes_labels[g][mcb_tuple[i][t]])
                ...
                for m in range(len(mcb_tuple)):
                    for i in edges[e]:
                        if i in mcb_tuple[m]:
                            ...
                            edge_e = ((self.nodes_labels[g][edges[e][0]], self.nodes_labels[g][edges[e][1]]), weight_e)
                            weight_m = tuple(self.find_ring_weights(mcb_tuple[m], g_list[g]))
                            ring_m = []
                            for t in range(len(mcb_tuple[m])):
                                ring_m.append(self.nodes_labels[g][mcb_tuple[m][t]])
